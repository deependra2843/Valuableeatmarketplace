...
const DayComponent = ({ items,updateAmount,option,fetchDataForDay }) => {
  console.log(items);
  ...
  const addToCart = async (restaurantId,price) => {
    const searchParams = new URLSearchParams(location.search);
    const city = searchParams.get('city');
    try {
      if(option.day===true){
        console.log("hill");
      fetchDataForDay('1',price);
      }
    ...
    <div>
      {filteredItems.map((item, index) => (
        <div key={index} className="row ml-auto mr-0 mt-3">
          ...
          <button type="button" className="add-btn" onClick={() => addToCart(item._id,item.price)}>Add Meal</button>
          </div>
        </div>
      ))}
